...
func getUsers(w http.ResponseWriter, r *http.Request) {
    ...
    rows, err := db.Query("SELECT Id, Name, Lastname, Username, Email FROM users")
    if err != nil {
    ...
    for rows.Next() {
        var user User
        if err := rows.Scan(&user.ID, &user.Name, &user.Lastname, &user.Username, &user.Email); err != nil {
...
func createUser(w http.ResponseWriter, r *http.Request) {
    ...
    _, err = db.Exec("INSERT INTO users (Name, Lastname, Username, Email) VALUES (?, ?, ?, ?)",
...
func updateUser(w http.ResponseWriter, r *http.Request) {
    ...
    _, err = db.Exec("UPDATE users SET Name=?, Lastname=?, Username=?, Email=? WHERE Id=?",
        user.Name, user.Lastname, user.Username, user.Email, user.ID)
    if err != nil {
